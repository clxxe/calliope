...
calliope::menu_t calliope::menu;

bool calliope::util::key_state[ 256 ];
bool calliope::util::prev_key_state[ 256 ];

// @NOTE: chloe 
// HI HELLO REPLACE THE KEYSTATES BELOW WITH UNITY KEYS 
// UnityEngine.Input <-- USE THAT (GOOGLE) 
// Same for the GetCursorPos and ScreenToClient replace that with UnityEngine mousePosition

void calliope::util::get_key_states( ) {
	for ( int i{ 0 }; i < 256; i++ ) {
...
bool calliope::util::is_key_pressed( int key_code ) {
	return key_state[ key_code ] && !prev_key_state[ key_code ]; //GetAsyncKeyState( key_code ) & 1;
}

bool calliope::util::is_key_held( int key_code ) {
...
bool calliope::menu_t::components_t::window( const char* title, vec2_t position, vec2_t window_size, int flags ) {
	if ( calliope::util::is_key_pressed( VK_INSERT ) && !( flags & window_flags::window_flags_always_open ) )
	...
	menu.mouse_active = GetAsyncKeyState( VK_LBUTTON ) & 0x8000;

	POINT m_point{ };
	GetCursorPos( &m_point );
	ScreenToClient( FindWindowA( "UnityWndClass", nullptr ), &m_point );
	...
	if ( menu.window_ctx.is_open ) {
		...
		if ( !( flags & window_flags::window_flags_no_titlebar ) ) {
			...
			menu.drawing.text( menu.window_ctx.position + vec2_t( menu.window_ctx.size.x - ( text_sz.x + 5 ), 2 ), title, globals::menu_accent, text_flags::text_flags_style_dropshadow );
...
#ifdef CALLIOPE_USE_TEXT_SIZE
		menu.drawing.multi_color_filled_rect( menu.window_ctx.position + vec2_t( 0, menu.window_ctx.size.y - 7.f ), { menu.window_ctx.size.x, 6 }, { globals::palette::black.alpha( 0 ), globals::palette::black.alpha( 0 ), color_t( 8, 8, 8, 255 ), color_t( 8, 8, 8, 255 ) } );

		menu.window_ctx.drawable_area.second.y -= 2.f;
#endif
...
int calliope::menu_t::components_t::tabs( std::vector< const char* > tab_names ) {
	...
	vec2_t tab_name_start = menu.window_ctx.position + vec2_t( 5, 2 );
	...
	for ( int i{ }; i < tab_names.size( ); i++ ) {
		...
		tab_name_start.x += ( tab_name_sz.x + 10 );
	}
#endif // CALLIOPE_USE_TEXT_SIZE
