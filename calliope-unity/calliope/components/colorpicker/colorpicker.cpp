...
calliope::color_t hex_to_rgb( std::string hex ) {
    ...
    out = calliope::color_t( std::stoi( colori[ 0 ], nullptr, 16 ),
        std::stoi( colori[ 1 ], nullptr, 16 ),
        std::stoi( colori[ 2 ], nullptr, 16 ),
        std::stoi( colori[ 3 ], nullptr, 16 )
    );

    return out;
}

void set_clipboard_text( const char* value ) {
    ...
    memcpy( dst, value, len * sizeof( char ) );
    dst[ len ] = '\0';
...
const char* get_clipboard_text( ) {
    ...
    if ( auto handle = GetClipboardData( CF_TEXT ) ) {
        out = static_cast< char* >( GlobalLock( handle ) );
...
void calliope::menu_t::components_t::colorpicker( const char* title, color_t& color, bool inlined ) {
    ...
    const bool preview_hovered = menu.mouse_in_rect( 
        menu.m_current_groupbox->position + color_preview_position + vec2_t( 0, calliope::globals::group_box_padding + 15 ),
        menu.m_current_groupbox->position + color_preview_position + color_preview_size + vec2_t( 0, calliope::globals::group_box_padding + 15 ) );

    auto color_picker_position = menu.m_current_groupbox->position + color_preview_position + color_preview_size + vec2_t( 0, color_preview_size.y + 16 );
    auto color_picker_size = vec2_t( 180, 175 );

    const bool picker_window_hovered = menu.mouse_in_rect(
        color_picker_position,
        color_picker_position + color_picker_size );
    ...
    if ( preview_hovered && ( util::is_key_pressed( VK_RBUTTON ) ) && was_empty_last_hash ) {
        ...
        static constexpr const char* m_titles[ ] = { "Copy", "Paste" };

        char color_buf[ 256 ]{ };

        sprintf_s( color_buf, "#%02X%02X%02X%02X",
        ...
        for ( int idx = 0; idx < 2; idx++ ) {
            ...
            if ( hovered && ( menu.mouse_active && !menu.old_mouse_active ) ) {
                if ( idx == 0 ) {
                    ...
                    set_clipboard_text( color_buf );
                }
                else {
                    if ( const auto txt = get_clipboard_text( ) ) {
                        color = hex_to_rgb( txt );
                    }
                }
        ...
        for ( int i{ }; i < 6; ++i ) {
            menu.draw_list.multi_color_filled_rect(
                hue_bar_position + vec2_t( 0, i * ( hue_bar_size.y / 6.f ) ),
                vec2_t( hue_bar_size.x, hue_bar_size.y / 6.f ),
                {
                    hue_colors[ i ],
                    hue_colors[ i ],
                    hue_colors[ i + 1 ],
        ...
        auto picker_mouse_cursor = vec2_t(
            std::clamp( picker_position.x + std::roundf( color_hsv.y * picker_position_size.x ), picker_position.x, picker_position.x + picker_position_size.x ),
            std::clamp( picker_position.y + std::roundf( ( 1.f - color_hsv.z ) * picker_position_size.y ), picker_position.y, picker_position.y + picker_position_size.y )
        );

        auto hue_bar_mouse_cursor = vec2_t( hue_bar_position.x, std::clamp( hue_bar_position.y + std::roundf( color_hsv.x * hue_bar_size.y ), hue_bar_position.y, hue_bar_position.y + hue_bar_size.y ) );
        ...
        auto alpha_bar_mouse_cursor = vec2_t( alpha_bar_position.x + std::roundf( color.a * alpha_bar_size.x ), alpha_bar_position.y );
        ...
        menu.draw_list.multi_color_filled_rect(
            picker_position,
            picker_position_size,
            {
                globals::palette::white,
                picker_color,
                picker_color,
                globals::palette::white
            }
    ...
    menu.drawing.filled_rect( color_preview_position, color_preview_size, color, 0.f );
    ...
    if ( !inlined ) {
        ...
        auto text_position = vec2_t( menu.m_current_groupbox->cursor_position.x, color_preview_position.y + ( ( color_preview_size.y * 0.5f ) - ( text_sz.y * 0.5f ) ) );

        menu.drawing.text( text_position, title, globals::palette::white, text_flags::text_flags_none );
        
        menu.m_current_groupbox->cursor_position.y += ( color_preview_size.y * 2.f ) + 2.f;
    }
}