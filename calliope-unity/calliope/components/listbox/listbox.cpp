...
void calliope::menu_t::components_t::begin_listbox( const char* name, std::vector< const char* >& items, int& value, float height ) {
	...
	int offset = static_cast< int >( current_listbox->old_cursor_position.y ) - static_cast< int >( current_listbox->size.y );
	...
	current_listbox->cursor_position.x -= current_listbox->position.x;
	current_listbox->cursor_position.y -= current_listbox->position.y + 20;
	...
	for ( int i{ }; i < items.size( ); i++ ) {
		const bool item_hovered = menu.mouse_in_rect(
			menu.m_current_groupbox->position + current_listbox->cursor_position + vec2_t( 0, current_listbox->position.y + calliope::globals::group_box_padding + 15.f + 20.f ),
			menu.m_current_groupbox->position + current_listbox->cursor_position + vec2_t( current_listbox->size.x, current_listbox->position.y + calliope::globals::group_box_padding + 15 + 20.f + 18.f )
		);

		const bool is_visible = current_listbox->cursor_position.y < ( current_listbox->size.y - 20 ) && current_listbox->cursor_position.y > -1;
		...
		if ( value == i ) {
			menu.drawing.filled_rect( vec2_t( 1, current_listbox->cursor_position.y ), vec2_t( 2, 20 ), globals::menu_accent, 0.f );
		}

		auto item_text_sz = menu.drawing.calc_text_size( items.at( i ) );
		menu.drawing.text( vec2_t( 6, current_listbox->cursor_position.y + ( 10 - ( item_text_sz.y / 2 ) ) ), items.at( i ), value == i ? globals::palette::white : color_t( 54, 54, 54, 255 ), text_flags::text_flags_none );

		current_listbox->cursor_position.y += 20;
	}

	menu.drawing.rect( vec2_t( 0, 0 ), vec2_t( current_listbox->size.x, height + 2 ), color_t( 0, 0, 0, 255 ), 0.f );
...
void calliope::menu_t::components_t::end_listbox( ) {
	...
	if ( ( listbox_ctx->m_item_count * 20 ) > listbox_ctx->size.y ) {
		if ( listbox_ctx->scroll_ratio == -1.f )
			listbox_ctx->scroll_ratio = 0.f;

		int scroll_height = ( listbox_ctx->size.y ) / ( ( listbox_ctx->old_cursor_position.y ) / ( listbox_ctx->size.y ) );
		int offset = ( ( listbox_ctx->size.y - 15 ) - scroll_height ) * listbox_ctx->scroll_ratio + 1;

		menu.drawing.filled_rect( vec2_t( listbox_ctx->size.x - 6, offset ), vec2_t( 4, 3 ), color_t( 32, 32, 32, 255 ), 0.f );
		menu.drawing.filled_rect( vec2_t( listbox_ctx->size.x - 6, offset ), vec2_t( 4, scroll_height - 6 ), color_t( 32, 32, 32, 255 ), 3.f );
		menu.drawing.filled_rect( vec2_t( listbox_ctx->size.x - 6, offset ), vec2_t( 1, scroll_height - 6 ), color_t( 32, 32, 32, 255 ), 0.f );

		bool hovered = menu.mouse_in_rect(
			menu.m_current_groupbox->position + listbox_ctx->position,
			menu.m_current_groupbox->position + listbox_ctx->position + listbox_ctx->size
		);

		if ( hovered ) {
			listbox_ctx->scroll_ratio = std::clamp( listbox_ctx->scroll_ratio - menu.mouse_wheel_delta * ( 20.f / ( ( int )listbox_ctx->old_cursor_position.y - ( int )( ( int )listbox_ctx->size.y ) ) ), 0.f, 1.f );
		}

		if ( listbox_ctx->scroll_ratio > 0.0f )
			menu.drawing.multi_color_filled_rect( vec2_t( 1, 0 ), vec2_t( listbox_ctx->size.x - 8, 22 ), { color_t( 8, 8, 8, 255 ), color_t( 8, 8, 8, 255 ), color_t( 8, 8, 8, 0 ), color_t( 8, 8, 8, 0 ) } );

		if ( listbox_ctx->scroll_ratio < 1.0f )
			menu.drawing.multi_color_filled_rect( vec2_t( 1, listbox_ctx->size.y - 41 ), vec2_t( listbox_ctx->size.x - 8, 22 ), { color_t( 8, 8, 8, 0 ), color_t( 8, 8, 8, 0 ), color_t( 8, 8, 8, 255 ), color_t( 8, 8, 8, 255 ) } );
	}
	else {
	...
	int offset = ( int )listbox_ctx->old_cursor_position.y - ( int )listbox_ctx->size.y;
	listbox_ctx->cursor_position.y += !( listbox_ctx->old_cursor_position.y > ( int ) listbox_ctx->size.y ) ? 0 : ( int )( offset * listbox_ctx->scroll_ratio );
