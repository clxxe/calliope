...
void calliope::menu_t::components_t::slider( const char* name, const char* suffix, value_t& value, value_t min, value_t max, std::optional< component_info_t > description ) {
	...
	bool hovered = menu.mouse_in_rect(
		menu.m_current_groupbox->position + slider_position + vec2_t( 0, calliope::globals::group_box_padding + 15 ),
		menu.m_current_groupbox->position + slider_position + slider_size + vec2_t( 0, calliope::globals::group_box_padding + 15 )
	);
	...
	if ( hovered && ( menu.mouse_active && !menu.old_mouse_active ) && was_empty_last_hash && util::is_key_held( VK_CONTROL ) ) {
		...
		if ( util::is_key_pressed( VK_ESCAPE ) || util::is_key_pressed( VK_RETURN ) || ( !hovered && menu.mouse_active ) ) {
			bool has_number = std::find_if( str_value.begin( ), str_value.end( ), ::isdigit ) != str_value.end( );
			...
			if ( has_number && minus_count < 2 && !str_value.empty( ) ) {
				if constexpr ( is_float_mode ) {
					value = std::clamp( std::stof( str_value ), static_cast< float >( min ), static_cast< float >( max ) );
				}
				else {
					value = std::clamp( static_cast< int >( std::stof( str_value ) ), static_cast< int >( min ), static_cast< int >( max ) );
				}
			}
		...
		if constexpr ( is_float_mode ) {
			value = static_cast< float >( std::clamp( menu.mouse_pos.x - ( menu.m_current_groupbox->position.x + slider_position.x ), static_cast< float >( min ), ( ( slider_size.x ) - 1 ) ) / ( slider_size.x - 1 ) * ( max - min ) + min );
			value = static_cast< float >( std::clamp( value, static_cast< float >( min ), static_cast< float >( max ) ) );
		}
		else {
			value = static_cast< int >( std::clamp( menu.mouse_pos.x - ( menu.m_current_groupbox->position.x + slider_position.x ), static_cast< float >( min ), ( slider_size.x - 1 ) ) / ( slider_size.x - 1 ) * ( max - min ) + min );
	...
	if constexpr ( is_float_mode )
		sprintf_s( buf, "%.2f%s", value, suffix );
	else
		sprintf_s( buf, "%d%s", value, suffix );

	if ( middle_aligned ) {
		menu.drawing.filled_rect( slider_position + vec2_t( slider_size.x / 2.f, 0 ), vec2_t( rect_width, slider_size.y ), calliope::globals::menu_accent, 0.f );
	}
	else {
		menu.drawing.filled_rect( slider_position, vec2_t( rect_width, slider_size.y ), calliope::globals::menu_accent, 0.f );
	}
	...
	menu.drawing.text( slider_position - vec2_t( 0, label_text_sz.y ), name, globals::palette::white, text_flags::text_flags_none );

	if ( !( menu.window_ctx.m_active_hash == ( util::hash( name ) + 2 ) ) ) {
		menu.drawing.text( slider_position + vec2_t( slider_size.x - value_buf_sz.x, -( value_buf_sz.y ) ), buf, globals::palette::white, text_flags::text_flags_none );
	}
	else {
		sprintf_s( buf, "%s", str_value.c_str( ) );
		auto value_buf_sz = menu.drawing.calc_text_size( buf );

		menu.drawing.text( slider_position + vec2_t( slider_size.x - value_buf_sz.x, -( value_buf_sz.y ) ), buf, globals::palette::white, text_flags::text_flags_none );

		// I should probably just remove this whole slider input idea....
		// Aids cursor blink meme
	...
	if ( description.has_value( ) ) {
		...
		auto end_of_slider_position = vec2_t(
			slider_position.x + label_text_sz.x + 3,
			slider_position.y - ( question_mark_sz.y ) );

		const bool hovered = menu.mouse_in_rect(
			menu.m_current_groupbox->position + end_of_slider_position + vec2_t( 0, calliope::globals::group_box_padding + 15 ),
			menu.m_current_groupbox->position + end_of_slider_position + question_mark_sz + vec2_t( 0, calliope::globals::group_box_padding + 15 ) );

		menu.drawing.text( end_of_slider_position, "(?)", hovered ? slider_hovered_description_color : slider_description_color, text_flags::text_flags_style_dropshadow );

		if ( hovered && menu.window_ctx.m_active_hash == 0 ) {
			auto overlay_window_position = menu.m_current_groupbox->position + end_of_slider_position + question_mark_sz + vec2_t( 4, slider_size.y * 2.f );
